(function( exports, require, console ){
  ...
  exports.optimizeScript = function( config, window, callback){
    ...
    async.map(srcs,
      function(item, callback){
        ...
        that.load(srcUrl, config.proxy, charset,
          function(err, body){
            exports.log(2, err);
            callback( null, body );
          }
  ...
  exports.getCssCharset = function(body){
    return (body.match(/^\@charset \"([^\"]+)\"/) || [])[1];
  };

  exports.getImportCssUrl = function(root, base, body){
    var imports = [],
        that = this;
    CSSOM.parse(body||'').cssRules.forEach(function(val){
      if(val.href) {
        imports.push(that.resolve( root, base, val.href ));
      }
    });
    imports = _(imports).compact();
    return imports;
  };

  exports.removeImportExpression = function(body){
    return (body.replace(/\@import\s+\"[^\"]+\";?\r?\n?/g, ""));
  };
  ...
  exports.optimizeCss = function(config, window, callback){
    var css = config.css || {},
        ...
        encoding = config.encoding || 'utf8',
        $ = window.$,
        links = $('link[href][rel=stylesheet],style').not(function(){
          var elem = $(this);
    ...
    async.mapSeries(links.get(), function(item, callback){
      var href = item.href,
      ...
      that.load(hrefUrl, config.proxy, encoding, function(err, body){
        var charset = that.getCssCharset(body) || encoding;

        async.mapSeries(that.getImportCssUrl(root, hrefUrl, body), function(url, callback){
          url = that.resolve( root, config.url, url );
          exports.log(1, "Import CSS: ");
          that.load(url, config.proxy, encoding, function(err, body){
            var charset = that.getCssCharset(body)||encoding;
            // body = that.convert(charset, '', body);
            callback(null, {
              config: config,
              url: url,
              body: body || ''
          ...
          callback(null, {
            config: config,
            url: hrefUrl,
            body: that.removeImportExpression( body || ''),
            children: results
          });
        });
      });
      ...
      results.forEach(function(result){
        results = results.concat( result.children||[] );
      });
      async.map(results, function(item, callback){
        base64 ? that.optimizeBackgroundImage( item.config, item.url, item.body, callback ) :
  ...
  exports.convertCSS = function(str){
    var charset = str.match(/^@charset \"([^\"]+)\"/);
    return charset ? new Iconv(charset[1], 'UTF-8').convert(str).toString('utf8') : str;
  };
  ...
  exports.optimize = function(config, callback ){
    ...
    this.load(config.url, config.proxy, null, function(err, body){
      fs.writeFileSync(config.dist+'/'+basename+'.unpack.html', body, 'utf8');
      jsdom.env({
        html: body,
        src: [
  ...
  exports.cli = function( config ){
    ...
    if( this.isLocal(config.url) && fs.statSync(config.url).isDirectory() ){
      if( fs.existFileSync ) {
        wrench.rmdirSyncRecursive(config.dist);
      }
      ...
      that.optimize( config, function(err){
        exports.log(4, "Finished!!");
        that.emit('exit', err);
      });
    }
  ...
  exports.emit = EventEmitter.prototype.emit;
