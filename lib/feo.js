...
exports.resolve = function( root, base, target ){
  if(this.isLocal(base)){
    return /^\//.test( target ) ? path.join( root, target ) :
...
exports.optimizeScript = function( config, window, callback){
  ...
  async.map(srcs,
    function(item, callback){
      ...
      that.isLocal(config.url) ? fs.readFile( srcUrl, 'utf8', function(err, body ){
        err && console.log(err);
        callback( null, body );
      }) : request({
        uri      : srcUrl,
...
exports.optimizeCss = function(config, window, callback){
  ...
  async.map(links.get(),
    function(item, callback){
      ...
      href = that.resolve( root, config.url, href );
      console.log("  "+ (href || "inline"));

      that.isLocal(config.url) ? 
