...
exports.optimizeScript = function( config, window, callback){
  var js = config.js || {},
      ...
      scripts = $('script')
                .not(function(){
                  var elem = $(this);
                  return ignore.indexOf( path.basename( this.src ) ) > -1 ? true : 
  ...
  async.map(srcs,
    function(item, callback){
      ...
      srcUrl = local ? path.resolve(path.dirname( config.url ), src) : url.resolve(config.url, src);
      ...
      local ? fs.readFile( srcUrl, 'utf8', function(err, body ){
        err && console.log(err);
        callback( null, body );
      }) : request({
        uri      : srcUrl,
...
exports.optimizeCss = function(config, window, callback){
  var css = config.css || {},
      ...
      links = $('link[href][rel=stylesheet],style')
          .not(function(){
              var elem = $(this);
              return ignore.indexOf( this.href ) > -1 ? true : 
  ...
  async.map(links.get(),
    function(item, callback){
      ...
      href = local ? path.resolve( path.dirname( config.url ), href) : url.resolve(config.url, href);
      console.log("  "+ (href || "inline"));

      local ? 
        fs.readFile( href, 'utf8', function(err, body){
          err && console.log(err);
          body = (body||'').replace(/[\"\']/g,"");
          that.optimizeBackgroundImage( config, href, body, callback );
        })
      : request({
          uri : href,
          encoding : 'utf8',
...
exports.optimizeBackgroundImage = function( config, href, body, callback){
  ...
  async.map(backgroundImages, function(item, callback){
    ...
    imageUrl = local ? path.resolve( path.dirname( href ), item ) : url.resolve( href, item );
...
exports.optimizeImage = function( config, window, callback ){
  ...
  async.map(imgs.get(),
    function(item, callback){
      ...
      imageUrl = local ? path.resolve( path.dirname( config.url ), src ) : url.resolve( config.url, src );
      console.log("  "+ imageUrl);
...
exports.cli = function( config ){
    ...
    config.local = this.isLocal( config.url );
    this.init( config );
